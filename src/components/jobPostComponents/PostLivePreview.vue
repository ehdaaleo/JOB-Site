    ...
    <div>
      ...
      <div class="bg-white border border-slate-200 rounded-xl p-[18px] shadow-[0_2px_8px_rgba(0,0,0,0.06)]">
        ...
        <div class="flex gap-[6px] flex-wrap mb-[12px]">
          <span v-if="form.work_type" class="px-[10px] py-[3px] bg-blue-50 border border-blue-100 text-blue-600 rounded-full text-[13px] font-bold tracking-wide capitalize">{{ form.work_type.replace('_', ' ') }}</span>
          <span v-if="form.experience_level" class="px-[10px] py-[3px] bg-blue-50 border border-blue-100 text-blue-600 rounded-full text-[13px] font-bold tracking-wide capitalize">{{ form.experience_level }}</span>
        ...
        <div class="text-[15px] font-bold text-green-600 truncate">
          <span v-if="form.salary_min || form.salary_max">
            ${{ form.salary_min ? Number(form.salary_min).toLocaleString() : '?' }}
            –
            ${{ form.salary_max ? Number(form.salary_max).toLocaleString() : '?' }}
          </span>
