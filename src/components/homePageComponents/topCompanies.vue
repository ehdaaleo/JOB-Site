...
onMounted(async () => {
  try {
    const res = await companyApi.list({ per_page: 6 })
...
const displayName = (c) => c.company_name || c.organization || c.name || 'Company'
</script>
...
<template>
  <section class="py-16 bg-white">
    <div class="container mx-auto px-4">
      ...
      <div v-else-if="companies.length === 0" class="text-center text-gray-500 py-8">
        No companies are hiring yet. Check back soon.
      </div>
        ...
        <div v-for="(company, index) in companies" :key="company.id" class="bg-white rounded-xl border border-gray-200 p-5 hover:border-blue-300 hover:shadow-md transition-all">
          <div class="flex items-center gap-3 mb-4">
            <div :class="`w-12 h-12 rounded-xl bg-gradient-to-br ${getColor(index)} flex items-center justify-center text-white font-bold text-lg`">
              {{ displayName(company).charAt(0) }}
            </div>
            <div>
              <h3 class="font-semibold text-gray-900">{{ displayName(company) }}</h3>
              <p class="text-xs text-gray-500">{{ company.location || 'Unspecified' }}</p>
            </div>
          </div>

          <div class="flex gap-4 mb-4">
            <div class="text-center">
              <p class="font-bold text-gray-900 text-lg">{{ company.approved_jobs_count ?? 0 }}</p>
              <p class="text-xs text-gray-500">Jobs</p>
            </div>
            <div class="text-center">
              <p class="font-bold text-gray-900 text-lg">{{ company.company_size || '—' }}</p>
              <p class="text-xs text-gray-500">Size</p>
            </div>
          </div>

          <div v-if="company.industry" class="mb-4">
